...
export function CatalogPicker({ open, onClose, onPick, only }: { open: boolean; onClose: () => void; onPick: (p: CatalogPick) => void; only?: "product" | "service" }) {
  ...
  return (
    <Dialog open={open} onClose={onClose} title="Agregar del catálogo" description="Busque por nombre, código, marca o categoría.">
      ...
      <div className="mt-3 max-h-[55vh] space-y-1 overflow-y-auto">
        ...
        {!loading && !products.length && !services.length && <p className="p-3 text-sm text-slate-500">Sin resultados. Puede crear el producto o servicio en su módulo.</p>}
        {products.map((p) => (
